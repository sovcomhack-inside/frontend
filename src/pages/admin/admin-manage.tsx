...
import { User } from 'shared/api/authorization-service'
...
export const AdminManagePage: React.FC = (props) => {
  ...
  const mapUser = (user: User) => {
    return (
      <div
        className="bg-neutral-900 px-8 py-2 rounded-sm hover:bg-neutral-800 hover:cursor-pointer"
  ...
  const banUser = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const response = await fetchAPI.post<any>('/api/v1/admin/list_users', {
      email_in: [e.currentTarget.email.value],
    })
    fetchAPI.post('/api/v1/admin/update_user_status', {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        >
          ...
          <WhiteButton type="wide" value="Заблокировать" />
        </form>
      </div>
    </div>
  )
